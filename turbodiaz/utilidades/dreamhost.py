...
def moverArchivoProducto(file, id):
    if existeArchivoMedia(file)==True:
        ...
        shutil.move(f'{RUTA}turbodiaz/media/{file}', f'{RUTA2}assets/upload/producto/{nombre}')
...
def moverArchivoProducto2(file):
    shutil.move(f'{RUTA}turbodiaz/media/producto/{file}', f'{RUTA2}assets/upload/producto/{file}')
...
def existeArchivo(carpeta, archivo):
    try:
        ruta=f"{RUTA}turbodiaz/assets/upload/{carpeta}/{archivo}"
        fileObj = Path(ruta)
...
def existeArchivoMedia(archivo):
    try:
        ruta=f"{RUTA}turbodiaz/media/{archivo}"
        fileObj = Path(ruta)
        return fileObj.is_file()
    except Exception as e:
        return False
