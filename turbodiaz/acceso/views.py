...
def acceso_registro(request):
    ...
    if request.method =='POST':
        if form.is_valid():
            ...
            UsersMetadata.objects.create(correo=request.POST['correo'],telefono=request.POST['telefono'], direccion='', estado_id=2, pais_id=1, perfiles_id=1, user_id=u.id, genero_id=3, slug = slugify(nombre))
            token=utilidades.getToken({'id': u.id, 'time':int(time.time())})
            #token='123455'
            url=f"{settings.BASE_URL}acceso/verificacion/{token}"
            html=f"""Hola {request.POST['nombre']} {request.POST['apellido']}, te has registrado correctamente en https://turbodiaz.cl/. Estás a punto de completar tu registro, por favor haz clic en el siguiente enlace para terminar el proceso, o cópialo y pégalo en la barra de direcciones de tu navegador favorito:
                    <br />
                    <br />
                    <a href="{url}">{url}</a>
                """
            utilidades.sendMail(html, 'Turbo Diaz', request.POST['correo'])
